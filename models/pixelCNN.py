...
class pixelCNN(GenBase):
    ...
    def log_prob(self, x):
        '''
        Calculates the log probability of input x
        ...
        Returns
        -------
        log p(x): torch.FloatTensor, shape = (batch_size)

        '''

        logp = self.forward(x)[:,1] * x + self.forward(x)[:,0] * (1 - x)
        return torch.sum(logp, axis = (1,2,3))
